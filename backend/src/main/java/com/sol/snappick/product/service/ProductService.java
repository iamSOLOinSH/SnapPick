...
public class ProductService {
	...
	public ProductDetailRes createProduct(
		...
		//이미지 처리 및 저장
		if ( images != null ) {
			List<ProductImage> productImages = uploadImagesToMinio(images,
																   productToCreate);
	...
	@Transactional
	public List<ProductSimpleRes> readStoreProducts(Integer storeId) throws Exception {
	...
	public ProductDetailRes updateProduct(
		...
		productToUpdate.updateDetails(productCreateReq.getName(),
		...
		if ( images != null && images.length > 0 ) {
			...
			// 새 이미지 추가
			List<ProductImage> newImages = uploadImagesToMinio(images,
															   productToUpdate);
			productToUpdate.getImages()
						   .addAll(newImages);
