...
| 2.2   | 사용자 로그인                          | 3,000여명의 사용자 계정 생성 |
| 2.3.2 | 은행코드 조회                          | 상품 등록을 위한 은행코드 조회 |
| 2.4.1 | 수시입출금 상품 등록                   | 18개 각 은행마다 1개의 수시입출금 상품 등록 |
| 2.4.2 | 수시입출금 상품 조회                   | 계좌 생성을 위한 상품 목록 확인 |
| 2.4.3 | 계좌 생성                              | 각 사용자별 다수의 계좌 생성 |
| 2.4.4 | 계좌 목록 조회                         | 오픈뱅킹 서비스 |
| 2.4.5 | 계좌 조회 (단건)                       | 특정 계좌의 잔액과 정보 확인 용도 |
| 2.4.6 | 예금주 조회                            | 본인 명의의 계좌인지 확인하는 용도 |
| 2.4.9 | 계좌 입금                              | 초기 사용자별 계좌의 예금액을 늘리기 위한 용도 |
| 2.4.10| 계좌 이체                              | 서비스에서 결제하는 방법 |
| 2.4.12| 계좌 거래 내역 조회                    | 자신의 계좌 거래내역을 조회하기 위해 |
| 2.4.13| 계좌 거래 내역 조회 (단건)             | 계좌 입금 후 그 거래로 인해 발생한 거래 내역을 조회하기 위해 |
| 2.4.13| 계좌 해지                              | 사용자에게 생성한 계좌를 해지하기 위해 |
| 2.8   | 1원 인증                               | 본인 인증 |

### **서비스에서 직접 호출하는 금융 API**

| 번호  | API 명칭                               | 설명 |
|-------|----------------------------------------|------|
| 2.2   | 사용자 로그인                          | 사용자 로그인 |
| 2.4.3 | 계좌 생성                              | 판매자 회원가입 시 |
| 2.4.4 | 계좌 목록 조회                         | 내 계좌 목록 조회 |
| 2.4.5 | 계좌 조회 (단건)                       | 주계좌의 정보와 금액 확인용 |
| 2.4.6 | 예금주 조회                            | 판매자가 돈 보낼 때 자신의 계좌인지 확인용 |
| 2.4.10| 계좌 이체                              | 서비스 결제 시 사용 |
| 2.4.12| 계좌 거래 내역 조회                    | 1원 송금 후, 거래내역 확인용 |
| 2.8   | 1원 인증                               | 본인 인증 |
